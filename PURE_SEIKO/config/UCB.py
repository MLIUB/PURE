...
def aesthetic():
    ...
    config.train.osm_alpha = 1e-3   # this is the most important hyperparameter, a big alpha corresponds to stronger bonus
    config.train.osm_lambda = 1e-3  # this is just to ensure cov mat is invertible, not very important
    config.train.osm_clipping = 10  # this is not important, UCB bonus should not be big (probably < 1)
    ...
    # Set the environment variable for visible CUDA devices if not already set
    ...
    config = set_config_batch(
                config,
                total_samples_per_epoch=32*2,
                total_batch_size=16*2,
...
def get_config(name):
    # Retrieve the configuration function by name and execute it
    return globals()[name]()
